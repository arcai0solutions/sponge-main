...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
    position = 'right',
    colors = ['#1a1a1a', '#E31E24'], // Dark background, Red accent
    items = [
        { label: 'Home', ariaLabel: 'Go to home page', link: '/' },
        { label: 'About', ariaLabel: 'Learn about us', link: '/about' },
        { label: 'Services', ariaLabel: 'View our services', link: '/services' },
        { label: 'Projects', ariaLabel: 'View our projects', link: '/projects' }, // Placeholder for now
    ...
    className,
    logoUrl = '/new-logo.jpeg',
    menuButtonColor = '#fff',
    openMenuButtonColor = '#000', // Black when open on white panel
    ...
    useLayoutEffect(() => {
        const ctx = gsap.context(() => {
            ...
            if (toggleBtnRef.current) gsap.set(toggleBtnRef.current, { color: 'var(--foreground)' }); // Use CSS variable or let CSS handle it? Let's just default to white/black based on state in CSS.
            // Actually, better to just let GSAP handle it if enabled.
    ...
    const handleNavigation = useCallback((href: string) => {
        ...
        closeMenu();
        // Wait for close animation (approx 350ms) before navigating
        setTimeout(() => {
            router.push(href);
        }, 350);
            ...
            >
                ...
                >
                    {(() => {
                        ...
                        // If only 2 colors were passed (like in prompt), we might want to duplicate or generate variants if 3 layers are expected?
                        // The prompt had 2 colors. The logic below slices if length >= 3. 
                        // Let's just use what is passed.
                        return raw.map((c, i) => (
                            <div
                                key={i}
                ...
                <header
                    className="staggered-menu-header absolute top-0 left-0 w-full flex items-center justify-between p-6 md:p-8 lg:p-10 bg-transparent pointer-events-none z-20"
                    aria-label="Main navigation header"
                >
                    {/* Logo Section - Visible when menu is CLOSED or OPEN. 
              The prompt's CSS sets logo invert on open if background is white. 
